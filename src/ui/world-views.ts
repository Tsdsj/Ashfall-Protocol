import { REGION_STORIES } from "../narrative/briefing";
...
export function mapView(sim: Simulation) {
  return `<div class="map-layout"><section><div class="panel-title"><div><h2>灰谷自治区</h2><p>已发现 ${sim.state.discovered.length} / ${sim.gen.pois.length} 处地点。滚轮缩放、拖动平移；单击设置导航标记，再次单击同一处可清除。${sim.creative ? "创造模式：右键地图任意位置可传送。" : ""}</p></div></div><div class="button-row"><button class="secondary" data-map-control="in">放大 +</button><button class="secondary" data-map-control="out">缩小 −</button><button class="secondary" data-map-control="center">定位自己</button><button class="quiet" data-map-control="reset">全图</button>${sim.creative ? `<button class="secondary" data-map-control="reveal">解锁全部迷雾</button>` : ""}</div><div class="map-legend"><span><i></i>已探索</span><span><i style="background:var(--warning)"></i>无线电事件</span><span><i style="background:var(--ink)"></i>你的位置</span></div><div class="map-frame"><canvas id="world-map" aria-label="灰谷世界地图，显示探索区域、当前位置与世界事件"></canvas><div class="map-scale">256 m / 格 · GREYVALE SURVEY 2037</div></div></section><aside class="map-sidebar"><div class="column-heading"><strong>区域档案</strong><span>危险等级</span></div><div class="region-list">${REGIONS.map((r) => `<div class="region-row"><h3>${r.name}</h3><small class="mono">${r.english}</small><div class="region-risk">${[1, 2, 3, 4, 5].map((n) => `<i class="risk-dot ${n <= r.danger ? "filled" : ""}"></i>`).join("")}<span style="margin-left:8px">${r.danger} / 5</span></div><p class="field-help">${escapeHtml(REGION_STORIES[r.id] ?? "")}</p></div>`).join("")}</div></aside></div>`;
}

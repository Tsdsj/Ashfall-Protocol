...
export function mainMenu(hasSave: boolean, backend: string): string {
  return `<div class="menu"><div class="menu-top">${brand()}<div class="world-status"><i class="status-dot"></i><span>GREYVALE · 2037</span></div></div><div class="menu-main"><div class="menu-kicker">封锁线内，仍有生命。</div><h1 class="game-title">ASHFALL<span>PROTOCOL</span></h1><div class="chinese-title">灰烬协议</div><p class="menu-description">一份未完成的撤离名单。<br>一片不再回应的森林。<br>在灰谷，活下去是你自己的协议。</p><nav class="menu-nav" aria-label="游戏主菜单">${hasSave ? '<button class="primary" data-action="continue"><span>▸</span>继续生存<span class="arrow">↗</span></button>' : ""}<button class="${hasSave ? "" : "primary"}" data-action="new-game"><span>${hasSave ? "01" : "▸"}</span>进入灰谷<span class="arrow">↗</span></button><button data-action="load-menu"><span>02</span>读取记录</button><button data-action="settings"><span>03</span>设置</button><button data-action="credits"><span>04</span>关于灰烬协议</button></nav></div><div class="scene-caption"><strong>松谷镇 · 林务站</strong><div class="caption-line"></div><p>封锁第 1,094 天<br>北纬 47° 16′ · 东经 08° 31′</p></div><footer class="menu-footer"><span>单人生存 · 本地存档 · 键盘与鼠标</span><span class="build-tag">${backend} / 1.0 · GREYVALE ARCHIVE</span></footer></div>`;
}
